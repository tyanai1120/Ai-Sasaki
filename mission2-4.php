...
//削除機能
$filnename='mission_2-1_Ai Sasaki.txt';//ファイルを指定
...
$editnumber=$_POST['editnumber'];
	if(!empty($_POST['編集対象番号']))//もし編集対象番号が空ではない時
{
		foreach($lines as $line){//ループ
		$pieces=explode("<>",$line);
 		if($edit ==$pieces[0])//編集対象番号と投稿番号が一致したら
 {
 	$date0=$pieces[0];
 	$date1=$pieces[1];//なまえ
 	$date2=$pieces[2];//コメント
...
if(!empty($_POST['コメント'])&&!empty($_POST['名前']))//コメントと名前が入力されたら
	{
		if(empty($_POST['editnumber'])){//editnumberが空な時
	...
	else//編集対象番号が浮かび上がってる時
	{
		$arr=array();
		$lines=file($filename);
		foreach($lines as $line){
		$pieces=explode("<>",$line);
		if($editnumber !==$pieces[0])//イコールではない
{
	$arr[]=$line;//入力された投稿番号以外のものを入れる
}
else
{
		...
		$name=$_POST['名前'];//取得する名前の指定
		$comment=$_POST['コメント'];//取得するコメントを指定
		$arr[]=$pieces[0]."<>".$name."<>".$comment."<>".$pieces[3];//<>を各文字の間に入れる
		//echo $name;
...
<html>
<head>
<title>mission_2-4</title><!-- 文書のタイトル -->
</head>
<meta charset="UTF-8">
<body> <!--文書の内容 -->
<form method="post" action="mission2-4.php">
